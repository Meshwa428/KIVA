...
void configureArduinoOta() {
  ...
  ArduinoOTA.onStart([]() {
    ...
    otaProgress = 0; // Initialize percentage progress
    // REMOVED: otaLastProgressBytes = 0;
    // REMOVED: otaLastTotalBytes = 0;
  ...
  ArduinoOTA.onEnd([]() {
    ...
    // REMOVED: Logic to update otaLastProgressBytes and otaLastTotalBytes on completion
  ...
  // ArduinoOTA.onProgress([](unsigned int progress_val, unsigned int total_val) {
  //   // REMOVED: otaLastProgressBytes = progress_val;
  //   // REMOVED: otaLastTotalBytes = total_val;

  //   if (total_val > 0) {
  //     otaProgress = (progress_val * 100) / total_val;
  //   } else {
  //     otaProgress = 0; 
  //   }
...
bool startBasicOtaUpdate() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    otaStatusMessage = "Wi-Fi not connected for OTA."; // This will be the initial message before prompt
    Serial.println("BasicOTA: " + otaStatusMessage);

    activatePromptOverlay("OTA Wi-Fi",
                          "Connect Wi-Fi?",
                          "X", "O",
                          WIFI_SETUP_MENU, // Target menu if prompt is cancelled (stays on WIFI_SETUP_MENU)
                          []() { // Action on confirm
                            wifiConnectForScheduledAction = true;
                            postWifiActionMenu = OTA_BASIC_ACTIVE; // Go back to OTA_BASIC_ACTIVE after Wi-Fi connect
                            // Note: currentMenu will be set to WIFI_SETUP_MENU by prompt logic if confirmed
                          });
    return false; // Return false, prompt will handle next steps
  }

  // ---- If Wi-Fi is already connected ----
  resetOtaState(); // Ensure clean state if we proceed
  Serial.println("BasicOTA: Starting ArduinoOTA...");
  
  String basic_ota_password_to_use_for_display = "";
  if (isSdCardAvailable() && SD.exists(OTA_AP_PASSWORD_FILE)) {
    File pwFile = SD.open(OTA_AP_PASSWORD_FILE, FILE_READ);
    if (pwFile) {
      if (pwFile.size() > 0 && pwFile.size() < MAX_AP_PASSWORD_LEN + 5) {
        ...
        loaded_pw.trim();
        if (loaded_pw.length() >= MIN_AP_PASSWORD_LEN && loaded_pw.length() <= MAX_AP_PASSWORD_LEN) {
          basic_ota_password_to_use_for_display = loaded_pw;
        }
      }
      pwFile.close();
    }
  ...
  configureArduinoOta(); // Sets up callbacks, including onProgress

  // Construct otaDisplayIpAddress string (IP and Password ONLY)
  String displayInfo = WiFi.localIP().toString();
  if (!basic_ota_password_to_use_for_display.isEmpty()) {
      displayInfo += " P: " + basic_ota_password_to_use_for_display;
  }
  strncpy(otaDisplayIpAddress, displayInfo.c_str(), sizeof(otaDisplayIpAddress) - 1);
  otaDisplayIpAddress[sizeof(otaDisplayIpAddress) - 1] = '\0';
  ...
  // Set the initial main status message to the hostname
  otaStatusMessage = String(OTA_HOSTNAME) + ".local"; 
  // Or, if you prefer "Waiting for IDE..."
  // otaStatusMessage = "Waiting for IDE...";

  otaProgress = 0; // Indicates ready, waiting for IDE
  basicOtaStarted = true;
  Serial.printf("BasicOTA: Ready. Hostname: %s.local, Full Info for Display: %s\n", OTA_HOSTNAME, otaDisplayIpAddress);
  return true;
}
