...
void drawCustomIcon(U8G2& display, int x, int y, IconType iconType, IconRenderSize size) {
    ...
    if (size == IconRenderSize::SMALL) {
        switch (iconType) {
            case IconType::UI_CHARGING_BOLT:
                ...
                w = IconSize::SMALL_WIDTH; h = IconSize::SMALL_HEIGHT;
                icon_resolved = true;
                break;
    ...
    if (!icon_resolved) {
        ...
        switch (iconType) {
            case IconType::TOOLS: xbm_data = icon_tools_large_bits; break;
            case IconType::GAMES: xbm_data = icon_games_large_bits; break;
            case IconType::SETTINGS: xbm_data = icon_settings_large_bits; break;
            case IconType::INFO: xbm_data = icon_info_large_bits; break;
            case IconType::GAME_SNAKE: xbm_data = icon_game_snake_large_bits; break;
            case IconType::GAME_TETRIS: xbm_data = icon_game_tetris_large_bits; break;
            case IconType::GAME_PONG: xbm_data = icon_game_pong_large_bits; break;
            case IconType::GAME_MAZE: xbm_data = icon_game_maze_large_bits; break;
            case IconType::NAV_BACK: xbm_data = icon_nav_back_large_bits; break;
            case IconType::NET_WIFI: xbm_data = icon_net_wifi_large_bits; break;
            case IconType::NET_BLUETOOTH: xbm_data = icon_net_bluetooth_large_bits; break;
            case IconType::TOOL_JAMMING: xbm_data = icon_tool_jamming_large_bits; break;
            case IconType::TOOL_INJECTION: xbm_data = icon_tool_injection_large_bits; break;
            case IconType::SETTING_DISPLAY: xbm_data = icon_setting_display_large_bits; break;
            case IconType::SETTING_SOUND: xbm_data = icon_setting_sound_large_bits; break;
            case IconType::SETTING_SYSTEM: xbm_data = icon_setting_system_large_bits; break;
            case IconType::UI_REFRESH: xbm_data = icon_ui_refresh_large_bits; break;
            case IconType::UI_CHARGING_BOLT: xbm_data = icon_ui_charging_bolt_large_bits; break;
            case IconType::UI_VIBRATION: xbm_data = icon_ui_vibration_large_bits; break;
            case IconType::UI_LASER: xbm_data = icon_ui_laser_large_bits; break;
            case IconType::UI_FLASHLIGHT: xbm_data = icon_ui_flashlight_large_bits; break;
            case IconType::UTILITIES_CATEGORY: xbm_data = icon_utilities_category_large_bits; break;
            default: xbm_data = nullptr; break;
        }
    }
...
void drawRndBox(U8G2& display, int x, int y, int w, int h, int r, bool fill) {
    if (w <= 0 || h <= 0) return;
    if (w <= 2 * r || h <= 2 * r) {
        if (fill) display.drawBox(x, y, w, h);
        else display.drawFrame(x, y, w, h);
        return;
    }
    if (fill) display.drawRBox(x, y, w, h, r);
...
void updateMarquee(bool& marqueeActive, bool& marqueePaused, bool& marqueeScrollLeft,
    ...
    if (marqueePaused) {
        if (curTime - marqueePauseStartTime > 1200) { // Pause for 1.2 seconds
            marqueePaused = false;
    ...
    if (curTime - lastMarqueeTime > 70) { // Update every 70ms
