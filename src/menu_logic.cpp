  ...
  if (currentMenu == MAIN_MENU) {
    ...
    if (toolsCategoryIndex >= 0 && toolsCategoryIndex < getToolsMenuItemsCount()) {
      if (strcmp(toolsMenuItems[toolsCategoryIndex], "Jamming") == 0) {
        ...
        switch (toolsCategoryIndex) {
          case 0: currentMaxItemsInGrid = getInjectionToolItemsCount(); currentToolItemsListPtr = injectionToolItems; break;
          case 1: currentMaxItemsInGrid = getWifiAttackToolItemsCount(); currentToolItemsListPtr = wifiAttackToolItems; break;
          case 2: currentMaxItemsInGrid = getBleAttackToolItemsCount(); currentToolItemsListPtr = bleAttackToolItems; break;
          case 3: currentMaxItemsInGrid = getNrfReconToolItemsCount(); currentToolItemsListPtr = nrfReconToolItems; break;
...
void handleMenuBackNavigation() {
  ...
  bool menuChanged = true; // Assume menu will change unless explicitly set otherwise
  ...
  if (currentMenu == OTA_WEB_ACTIVE) {
    ...
    // General back from sub-menus (Games, Tools, Settings) to Main Menu
    currentMenu = MAIN_MENU;
    if (previousMenuState == GAMES_MENU) mainMenuSavedIndex = 0; // Index of "Games" in mainMenuItems
    else if (previousMenuState == TOOLS_MENU) mainMenuSavedIndex = 0; // Index of "Tools"
    else if (previousMenuState == SETTINGS_MENU) mainMenuSavedIndex = 2; // Index of "Settings"
    ...
    menuChanged = false; // No defined back action for this menu
  }
