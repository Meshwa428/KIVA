...
// --- Payload Data (Copied from original implementation) ---
static const uint8_t IOS1[]{0x02,0x0e,0x0a,0x0f,0x13,0x14,0x03,0x0b,0x0c,0x11,0x10,0x05,0x06,0x09,0x17,0x12,0x16};
...
static const DeviceType android_models[] = {{0x0001F0}, {0x000047}, {0x470000}, {0x00000A}, {0x00000B}, {0x00000D}, {0x000007}, {0x000009}, {0x090000}, {0x000048}, {0x001000}, {0x00B727}, {0x01E5CE}, {0x0200F0}, {0x00F7D4}, {0xF00002}, {0xF00400}, {0x1E89A7}, {0xCD8256}, {0x0000F0}, {0xF00000}, {0x821F66}, {0xF52494}, {0x718FA4}, {0x0002F0}, {0x92BBBD}, {0x000006}, {0x060000}, {0xD446A7}, {0x038B91}, {0x02F637}, {0x02D886}, {0xF00000}, {0xF00001}, {0xF00201}, {0xF00209}, {0xF00205}, {0xF00305}, {0xF00E97}, {0x04ACFC}, {0x04AA91}, {0x04AFB8}, {0x05A963}, {0x05AA91}, {0x05C452}, {0x05C95C}, {0x0602F0}, {0x0603F0}, {0x1E8B18}, {0x1E955B}, {0x1EC95C}, {0x06AE20}, {0x06C197}, {0x06C95C}, {0x06D8FC}, {0x0744B6}, {0x07A41C}, {0x07C95C}, {0x07F426}, {0x0102F0}, {0x054B2D}, {0x0660D7}, {0x0103F0}, {0x0903F0}, {0xD99CA1}, {0x77FF67}, {0xAA187F}, {0xDCE9EA}, {0x87B25F}, {0x1448C9}, {0x13B39D}, {0x7C6CDB}, {0x005EF9}, {0xE2106F}, {0xB37A62}, {0x92ADC9}};
struct WatchModel { uint8_t value; };
static const WatchModel watch_models[] = {{0x1A},{0x01},{0x02},{0x03},{0x04},{0x05},{0x06},{0x07},{0x08},{0x09},{0x0A},{0x0B},{0x0C},{0x11},{0x12},{0x13},{0x14},{0x15},{0x16},{0x17},{0x18},{0x1B},{0x1C},{0x1D},{0x1E},{0x20}};
...
void BleSpammer::setup(App* app, BleManager* bleManager) {
    app_ = app;
...
void BleSpammer::start(BleSpamMode mode) {
    if (isActive_) {
        LOG(LogLevel::WARN, "BLESPAM", "Already active.");
        return;
    }

    LOG(LogLevel::INFO, "BLESPAM", "Attempting to start BLE Spam Mode: %d", (int)mode);

    if (!bleManager_->requestBle(BleManager::BleOwner::SPAMMER)) {
        LOG(LogLevel::ERROR, "BLESPAM", "Failed to acquire BLE resource.");
        return;
    }
    ...
    LOG(LogLevel::INFO, "BLESPAM", "Spam task created.");
}
...
void BleSpammer::stop() {
    ...
    if (spamTaskHandle_ != nullptr) {
        ...
        spamTaskHandle_ = nullptr;
        LOG(LogLevel::INFO, "BLESPAM", "Spam task deleted.");
    }
...
void BleSpammer::spamTaskWrapper(void* param) {
    static_cast<BleSpammer*>(param)->spamTask();
}

void BleSpammer::spamTask() {
    LOG(LogLevel::INFO, "BLESPAM_TASK", "Task started.");
    while(isActive_) {
        executeSpamPacket();
...
void BleSpammer::executeSpamPacket() {
    ...
    if (!pAdvertising) {
        LOG(LogLevel::ERROR, "BLESPAM_TASK", "pAdvertising is null!");
        return;
    }
...
// All other functions (getAdvertisementData, generateRandomMac, etc.) remain the same
// and are copied below for completeness.
